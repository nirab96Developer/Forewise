...
def _export_worklogs(db: Session) -> openpyxl.Workbook:
    ...
    for r_idx, row in enumerate(rows, 2):
        ...
        ws.cell(r_idx, 2, str(row[1]) if row[1] else "")
        ws.cell(r_idx, 3, str(row[2]) if row[2] else "")
        ws.cell(r_idx, 4, str(row[3]) if row[3] else "")
        ws.cell(r_idx, 5, str(row[4]) if row[4] else "")
        ws.cell(r_idx, 6, str(row[5]) if row[5] else "")
        ...
        ws.cell(r_idx, 13, str(row[12]) if row[12] else "")

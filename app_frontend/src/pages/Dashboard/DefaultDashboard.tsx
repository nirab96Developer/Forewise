      ...
      <div className="max-w-[1600px] mx-auto p-6 space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-fw-border p-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              {/* Avatar */}
              <div className="w-14 h-14 bg-gradient-to-br from-fw-green to-fw-green-dark rounded-full flex items-center justify-center text-white font-bold text-lg shadow-md">
                {userInfo.initials}
              </div>
              
              {/* User Info */}
              <div>
                <h1 className="text-xl font-bold text-fw-text">{userInfo.name}</h1>
                <div className="flex items-center gap-3 text-sm text-gray-500 mt-1">
                  <span className="flex items-center gap-1">
                    <User className="w-4 h-4" />
                    {getRoleDisplayName(userInfo.role)}
                  </span>
                  <span className="text-fw-border">|</span>
                  <span className="flex items-center gap-1">
            ...
            <div className="text-left text-sm text-gray-400">
              <div>חיבור אחרון:</div>
              <div className="font-medium text-fw-text">{userInfo.lastLogin}</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-12 lg:col-span-3 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-fw-border p-5">
              <div className="flex items-center gap-2 mb-3">
                <Calendar className="w-5 h-5 text-fw-green" />
              ...
              <button 
                ...
                className="w-full bg-fw-green hover:bg-fw-green-dark text-white py-3 rounded-lg flex items-center justify-center gap-2 transition-colors font-medium shadow-sm"
              >
            ...
            {/* Quick Actions */}
            <div className="bg-white rounded-xl shadow-sm border border-fw-border p-5">
              <h3 className="text-base font-semibold text-fw-text mb-4 flex items-center gap-2">
                <TrendingUp className="w-5 h-5 text-fw-green" />
                פעולות מהירות
              </h3>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-fw-border p-5">
              <h3 className="text-base font-semibold text-fw-text mb-4 flex items-center gap-2">
                <Building2 className="w-5 h-5 text-fw-green" />
                פרויקטים אחרונים
              </h3>
              ...
              <div className="space-y-2">
                {projects.slice(0, 4).map((project) => (
                  <button
                    ...
                    className="w-full text-right p-3 rounded-lg border border-fw-gray-light hover:border-fw-green hover:bg-fw-green-light/30 transition-all group"
                  >
                    <div className="font-medium text-fw-text group-hover:text-fw-green truncate">
                      {project.name}
                    </div>
          ...
          <div className="col-span-12 lg:col-span-6">
            <div className="bg-white rounded-xl shadow-sm border border-fw-border p-5">
              ...
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-semibold text-fw-green flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  <button 
                    ...
                    className="p-2 rounded-lg hover:bg-fw-green-light transition-colors"
                  >
                    <ChevronRight className="w-5 h-5 text-fw-green" />
                  </button>
                  <button 
                    onClick={() => setCurrentDate(new Date())}
                    className="px-3 py-1 text-sm bg-fw-green-light text-fw-green rounded-lg hover:bg-fw-green hover:text-white transition-colors"
                  >
                    היום
                  </button>
                  <button 
                ...
                {calendarDays.map((day, index) => (
                  <button
                    ...
                    className={`
                      ...
                      ${day.isCurrentMonth ? 'hover:bg-fw-green-light' : 'text-gray-300'}
                      ${day.isToday ? 'bg-fw-green text-white font-bold hover:bg-fw-green-dark' : ''}
                      ${selectedDate?.getDate() === day.date && day.isCurrentMonth && !day.isToday 
                        ? 'bg-fw-green-light border-2 border-fw-green' 
                        : ''
                      }
                    `}
                  >
                    <span className={day.isCurrentMonth ? 'text-fw-text' : ''}>
                      {day.isToday ? day.date : day.date}
                    </span>
                    {day.hasEvents && day.isCurrentMonth && !day.isToday && (
                      <div className="absolute bottom-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-fw-green rounded-full" />
                    )}
                  </button>
                ))}
              </div>
              ...
              {/* Selected Day Events */}
              <div className="mt-6 pt-6 border-t border-fw-border">
                <h4 className="text-sm font-semibold text-gray-500 mb-3">
                  פעילויות ל-{selectedDate?.getDate()} {hebrewMonths[selectedDate?.getMonth() || 0]}
                </h4>
                <div className="space-y-2">
                  <div className="p-3 bg-fw-green-light rounded-lg flex items-center gap-3">
                    <div className="w-2 h-2 bg-fw-green rounded-full" />
                    <span className="text-sm text-fw-text">אין פעילויות מתוכננות</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="col-span-12 lg:col-span-3 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-fw-border p-5">
              <h3 className="text-base font-semibold text-fw-text mb-4 flex items-center gap-2">
                <CheckCircle className="w-5 h-5 text-fw-success" />
                סטטוס מערכת
              </h3>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="flex items-center gap-1 text-fw-success">
                    <span className="w-2 h-2 bg-fw-success rounded-full animate-pulse" />
                    פעיל
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
...
const QuickActionButton: React.FC<{
  ...
  <button
    onClick={onClick}
    className="w-full bg-white border border-fw-border px-4 py-3 rounded-lg flex items-center gap-3 text-fw-text hover:bg-fw-green-light hover:border-fw-green hover:text-fw-green transition-all group"
  >
    <span className="text-gray-400 group-hover:text-fw-green transition-colors">
      {icon}
    </span>
...
const StatusCard: React.FC<{
  ...
  const colorClasses = {
    green: 'text-fw-green bg-fw-green-light',
    yellow: 'text-fw-warning bg-yellow-50',
    blue: 'text-fw-info bg-blue-50',
    red: 'text-fw-error bg-red-50',
  };

  return (
    <button
      onClick={onClick}
      className="w-full bg-white rounded-xl shadow-sm border border-fw-border p-5 hover:shadow-md hover:border-fw-green transition-all text-right"
    >
      ...
      <div className="text-3xl font-bold text-fw-green">{count}</div>
    </button>
  );
};

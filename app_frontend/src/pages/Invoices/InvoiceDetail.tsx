...
showToast('החשבונית נשלחה לספק במייל ', 'success');
  ...
  if (error || !invoice) return (
    <div className="min-h-screen bg-fw-bg flex items-center justify-center" dir="rtl">
      <div className="text-center">
        ...
        <button onClick={() => navigate('/invoices')} className="mt-4 text-fw-green text-sm underline">
  ...
  return (
    <div className="min-h-screen bg-fw-bg print:bg-white" dir="rtl">
      <div className="max-w-4xl mx-auto px-4 py-6 print:py-2 print:px-0">

        {/* Header — hidden on print */}
        <div className="mb-6 print:hidden">
          <button onClick={() => navigate('/invoices')} className="text-fw-green text-sm flex items-center gap-1 mb-4">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-fw-text">חשבונית {invoice.invoice_number}</h1>
            ...
            <div className="flex gap-2">
              <button
                onClick={handlePrint}
                className="flex items-center gap-1.5 px-3 py-2 border border-fw-border rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition-colors"
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 border border-fw-border rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition-colors disabled:opacity-50"
              >
              ...
              {!isPaid && canApproveInvoice && (
                <button
                  ...
                  disabled={actionLoading}
                  className="flex items-center gap-1.5 px-4 py-2 bg-fw-green text-white rounded-lg text-sm hover:bg-fw-green-dark transition-colors disabled:opacity-50"
                >
        ...
        {/* Invoice Header Card */}
        <div className="bg-white rounded-xl shadow-sm border border-fw-border p-6 mb-4 print:shadow-none print:border print:rounded-none">
          {/* Print header */}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="flex items-start gap-3">
              <div className="w-9 h-9 bg-fw-green-light rounded-lg flex items-center justify-center flex-shrink-0">
                <Truck className="w-4 h-4 text-fw-green" />
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-0.5">ספק</p>
                <p className="text-sm font-semibold text-fw-text">{invoice.supplier_name || `ספק #${invoice.supplier_id}`}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-fw-text">{fmtDate(invoice.issue_date)}</p>
            ...
            <div className="flex items-start gap-3">
              <div className="w-9 h-9 bg-fw-green-light rounded-lg flex items-center justify-center flex-shrink-0">
                <span className="w-4 h-4 text-fw-green font-bold leading-none inline-flex items-center justify-center">₪</span>
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-0.5">יתרה לתשלום</p>
                <p className={`text-sm font-bold ${isPaid ? 'text-green-600' : 'text-fw-text'}`}>
...
{isPaid && <p className="text-xs text-green-600 font-medium"> שולם</p>}
        ...
        {/* Invoice Items Table */}
        <div className="bg-white rounded-xl shadow-sm border border-fw-border overflow-hidden mb-4 print:shadow-none print:border print:rounded-none">
          <div className="px-4 py-3 border-b border-fw-border bg-gray-50 flex items-center gap-2">
            <FileText className="w-4 h-4 text-fw-green" />
            <h2 className="text-sm font-semibold text-fw-text">פריטי חשבונית</h2>
              ...
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-fw-border">
                  <tr>
                    ...
                    <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500 bg-fw-green-light">סה"כ</th>
                  </tr>
                </thead>
                <tbody>
                  {items.map((item, idx) => (
                    <tr key={item.id} className={`border-b border-fw-border ${idx % 2 === 0 ? '' : 'bg-gray-50/50'}`}>
                      <td className="px-4 py-3 text-sm text-fw-text max-w-xs">
...
{item.hourly_rate != null ? `${item.hourly_rate}` : '—'}
                      ...
                      <td className="px-4 py-3 text-center bg-fw-green-light/30">
                        <span className="text-sm font-bold text-fw-text">{fmtILS(item.total_price)}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        {/* Totals Summary */}
        <div className="bg-white rounded-xl shadow-sm border border-fw-border p-5 print:shadow-none print:border print:rounded-none">
          <div className="max-w-sm mr-auto space-y-2">
            <div className="flex justify-between items-center text-sm">
              <span className="text-gray-600">סכום לפני מע"מ</span>
              <span className="font-medium text-fw-text">{fmtILS(invoice.subtotal)}</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-gray-600">מע"מ (18%)</span>
              <span className="font-medium text-fw-text">{fmtILS(invoice.tax_amount)}</span>
            </div>
            <div className="h-px bg-fw-border my-2" />
            <div className="flex justify-between items-center">
              <span className="text-base font-bold text-fw-text">סה"כ לתשלום</span>
              <span className="text-xl font-bold text-fw-green">{fmtILS(invoice.total_amount)}</span>
            </div>

...
const NewWorkOrder: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Business rule (Q1): each work order = one piece of equipment.
    // If the user requested N units, create N independent work orders so the
    // full lifecycle (supplier / equipment / worklog / invoice) is tracked
    // per-item.
    const buildPayload = (sequenceNumber: number): WorkOrderCreate => {
      const titleSuffix = quantityNumber > 1 ? ` (${sequenceNumber}/${quantityNumber})` : '';
      return {
        title: `דרישת כלי: ${formData.tool_type}${titleSuffix}`,
    ...
    const navigateBack = () => {
      if (projectCode) {
        ...
        import('../../services/api').then(m => m.default.get(`/projects/${projectIdFromUrl}`)).then(({ data }) => {
    ...
    try {
      if (!navigator.onLine) {
        // Offline — queue all N orders separately
        for (let i = 1; i <= quantityNumber; i++) {
          await saveOfflineWorkOrder(buildPayload(i));
        }
        const msg = quantityNumber > 1
          ? `${quantityNumber} ההזמנות נשמרו במכשיר — תועלינה כשיחזור חיבור`
          : ' ההזמנה נשמרה במכשיר — תועלה כשיחזור חיבור';
        showToast(msg, 'info', 7000);
        showToast(' זכור: אחרי סנכרון תצטרך לשלוח את ההזמנות לספק ידנית', 'warning', 8000);
        setTimeout(() => navigate('/pending-sync'), 1800);
        return;
      }

      // Create N independent work orders. Use Promise.allSettled so a single
      // failure doesn't block the others (and the user gets a clear count).
      const results = await Promise.allSettled(
        Array.from({ length: quantityNumber }, (_, i) => workOrderService.createWorkOrder(buildPayload(i + 1)))
      );
      ...
      if (failed === 0) {
        const msg = quantityNumber > 1
          ? `נוצרו ${succeeded} הזמנות נפרדות (כלי לכל הזמנה) — נשלחו לאישור מתאם`
          : 'ההזמנה נשלחה בהצלחה לאישור מתאם ההזמנות';
        showToast(msg, 'success');
        navigateBack();
      } else if (succeeded > 0) {
        showToast(`נוצרו ${succeeded}/${quantityNumber} הזמנות. ${failed} נכשלו — בדוק והגש מחדש את החסרות.`, 'warning', 8000);
        navigateBack();
      } else {
        // All failed — show first error
        const firstReject = results.find(r => r.status === 'rejected') as PromiseRejectedResult | undefined;
        const err: any = firstReject?.reason;
        const errorMessage = err?.response?.data?.detail || err?.response?.data?.message || 'שגיאה ביצירת דרישת כלים';
      ...
      if (!error.response) {
        ...
        showToast(' ההזמנות נשמרו במכשיר — תועלינה כשיחזור חיבור', 'info', 7000);
        showToast(' זכור: אחרי סנכרון תצטרך לשלוח את ההזמנות לספק ידנית', 'warning', 8000);
        setTimeout(() => navigate('/pending-sync'), 1800);
      } else {
...
שמירה: {formData.guard_days} לילות × {overnightGuardRate} = {(formData.guard_days * overnightGuardRate).toLocaleString('he-IL')}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-fw-border p-5">
            ...
            <div className="grid grid-cols-12 gap-4">
              ...
              {/* Quantity — creates N independent work orders (one per equipment unit) */}
              <div className="col-span-12 sm:col-span-6">
                <label className="block text-sm font-medium text-fw-text mb-2">
                  כמות כלים
                </label>
                ...
                {quantityNumber && quantityNumber > 1 && (
                  <p className="text-xs text-fw-info mt-1.5 flex items-start gap-1">
                    <Info className="w-3.5 h-3.5 mt-0.5 flex-shrink-0" />
                    <span>
                      תיווצרנה <strong>{quantityNumber} הזמנות נפרדות</strong> — כלי לכל הזמנה.
                      כל הזמנה תקבל ספק, מספר רישוי, דיווחים וחשבונית בנפרד.
                    </span>
                  </p>
                )}
              </div>

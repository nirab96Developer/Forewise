...
const NewRegion: React.FC = () => {
  ...
  const loadManagers = async () => {
    try {
      ...
      // Filter to region managers and admins
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || 'שגיאה ביצירת המרחב');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">מרחב חדש</h1>
              <p className="text-gray-500">הוסף מרחב גיאוגרפי חדש למערכת</p>
            </div>
          </div>
        </div>
...
תקציב כולל למרחב ()
              ...
              <p className="text-xs text-emerald-600 mt-2">
                התקציב הכולל שמוקצה למרחב זה. יוצג בכרטיסי הסטטיסטיקה.
              </p>
            </div>

...
<div className="font-medium mb-2">התעריף שונה מברירת המחדל של הקבוצה ({catDefaultRate}). להמשיך?</div>
              <div className="flex gap-2">
                <button onClick={()=>{setShowRateConfirm(false);save();}} className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-fw-green text-white hover:bg-fw-green-dark border-none cursor-pointer">כן, שמור</button>
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <button onClick={()=>setShowNewCat(!showNewCat)} className="flex items-center gap-0.5 text-xs text-fw-green hover:underline border-none bg-transparent cursor-pointer">
            ...
            {showNewCat && (
              <div className="flex gap-2 mb-2">
                <input value={newCatName} onChange={e=>setNewCatName(e.target.value)} placeholder="שם הקבוצה החדשה"
                  className="flex-1 px-3 py-2 rounded-xl border border-fw-green text-sm focus:outline-none" />
                <button onClick={createCat} className="px-3 py-2 bg-fw-green text-white rounded-xl text-sm font-semibold hover:bg-fw-green-dark border-none cursor-pointer">צור</button>
              </div>
            )}
          ...
          <div>
            ...
            <input value={form.name} onChange={e=>f('name',e.target.value)} placeholder="בחר קבוצה תחילה" disabled={!form.category_id}
              className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:border-fw-green disabled:bg-gray-50 disabled:text-gray-400" />
          </div>
...
<label className="block text-xs font-semibold text-gray-600 mb-1">תעריף שעתי () <span className="text-red-500">*</span></label>
              <input type="number" value={form.hourly_rate} onChange={e=>f('hourly_rate',e.target.value)} placeholder="—"
                className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:border-fw-green" />
            </div>
            <div>
<label className="block text-xs font-semibold text-gray-600 mb-1">תעריף לינה ()</label>
          ...
          <label className="flex items-center gap-3 cursor-pointer select-none">
            <input type="checkbox" checked={form.night_guard} onChange={e=>f('night_guard',e.target.checked)} className="w-4 h-4 rounded text-fw-green focus:ring-fw-green" />
          ...
          <div>
            ...
            <textarea value={form.notes} onChange={e=>f('notes',e.target.value)} rows={2} placeholder="בחר קבוצה תחילה"
              className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm placeholder-gray-400 resize-none focus:outline-none focus:border-fw-green" />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-gray-50 border-t border-gray-100">
          ...
          <button onClick={save} disabled={saving||!form.category_id||!form.name.trim()} className="px-5 py-2 rounded-xl bg-fw-green text-white text-sm font-semibold hover:bg-fw-green-dark transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
